...
ENGINE=$(cd "$(dirname "$0")/../engines/vivlio-docker" && pwd)
...
run() {  # run <in> <out> <theme>
	( cd "$BASE" && "$ENGINE/convert" "$1" "$2" "$3" 2>"$BASE/err" )
}
...
check "contract args are container paths" "/in/a.md /out/a.pdf /app/theme" \
      "$(contract "$OUT")"
check "the image is named" "yes" "$(has "$OUT" "tomgidden/pdfulator:vivlio")"
...
fixture
OUT=$(printf '# Hi\n' | ( cd "$BASE" && "$ENGINE/convert" - - "" 2>/dev/null ))
...
check "stdin stays stdin in the container" "- - /app/theme" "$(contract "$OUT")"
...
check "and the contract names both" "/in/a.md /out/a.pdf /app/theme" \
      "$(contract "$OUT")"
...
check "the built-in theme uses the image's copy" "/app/theme" \
      "$(printf '%s\n' "$OUT" | tail -1)"
...
check "the configured image is used" "yes" \
      "$(has "$OUT" "$(sed -n 's/^image=//p' "$ENGINE/engine.conf" | head -1)")"

fixture
PDFULATOR_IMAGE=example/local:test && export PDFULATOR_IMAGE
OUT=$(run "$BASE/src/a.md" "$BASE/out/a.pdf" "$BASE/dist/theme")
check "an override is honoured" "yes" "$(has "$OUT" "example/local:test")"
check "and replaces the default" "no" "$(has "$OUT" "tomgidden/pdfulator:vivlio")"
...
sh -c "set -e; PDFULATOR_DOCKER='$BASE/bin/docker' PDFULATOR_DIR='$BASE/dist' \
       '$ENGINE/convert' '$BASE/src/a.md' '$BASE/out/a.pdf' '$BASE/dist/theme'" \
	>/dev/null 2>&1
